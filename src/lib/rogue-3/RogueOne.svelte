<script lang="ts">
	import { notes, addNote } from '../notes-1/notes.svelte.ts'
	...
	let keysLock = $derived(actionButtons.length)
	let modalEl = $state()
	let messages = $state([])
	...
	async function handleEncounter(symbol) {
		actionText = `${symbol === '_' ? 'Walk' : 'Hit'} ${symbol === '_' ? '' : symbol}`
		actionButtons = []
	...
	<footer>
		<div class="p-4">
			<div class="flex items-center justify-between">
				<h3>Player Stats</h3>
				<div>
					<div class="badge {keysLock ? 'badge-error' : 'badge-success'}">
						{currentFloor}
					</div>
				</div>
			</div>
			...
			<div class="playerStats text-lg">
				...
				<!-- <p>Equickment: <span class="invObj">{equiObj}</span></p> -->
			</div>
			<div>
				<small>{actionText ? actionText : ''}</small>
				<!-- {#each actionButtons as { text, onClick }}
					<button class="btn" onclick={onClick}>{text}</button>
				{/each} -->
			</div>
